...
if btc.get('btc_price'):
    emoji = {'strongly_bullish':'🟢🚫','bullish':'🟡','bullish_avalanche':'📗','neutral':'⚪','bearish':'🐻','crashing':'💀'}
...
for c in scan.get('candidates', [])[:3]:
    ...
    mm = ''
    if m5:
        g = '🔴' if not m5.get('last_5m_green') else '🟢'
        mm = f\" 5m:{g}{m5.get('5m_vol_ratio',0):.1f}x\"
    s = c['score']
    print(f'  {c[\"symbol\"]:12s} score={s:6.1f}  vol={c.get(\"max_vol_spike_ratio\",0):>5.1f}x b/s={c.get(\"buy_sell_ratio\",0):>5.1f} ba={c.get(\"order_book_ratio\",0):>5.2f}{mm}')
...
if trade.get('closed'):
    ...
    ev = trade['evaluation']
    print(f'\\n⏭️  Skip short: {ev.get(\"reason\",\"\")}')
else:
    print('\\nℹ️  No short action')

# ── Stats ──
print()
print(f'📊 SHORT stats: {state.get(\"total_trades\",0)} trades | {state.get(\"wins\",0)}W / {state.get(\"losses\",0)}L')
pos = state.get('active_position')
if pos:
    opened = pos.get('opened_at','?')
    print(f'📌 Position: SHORT {pos[\"symbol\"]} @ \${pos[\"entry_price\"]} (score={pos.get(\"entry_score\",\"?\")}) opened {opened}')
" || true
